...
- [x] Add focused regression coverage for subtopic lesson-ready counts.
- [x] Show lesson-ready coverage in each grouped subtopic header.
- [x] Run focused Atlas/dashboard test, lint, build, and whitespace checks.
- [x] Commit and push to the private app repo.
- [ ] Retry production deploy when Vercel daily deploy limit resets.
...
- App commit: `981cce9 Show Atlas subtopic lesson readiness`, pushed to private `auronpep/barmatrix-app` `main`.
- UI change: each grouped Atlas subtopic header now distinguishes `q-ready` and `lesson-ready` counts.
- Proof passed: `node --test tests\ambassador-dashboard-entry.test.ts`, `git diff --check` (CRLF warnings only), `npm run lint`, and `npm run build`.
- Production deploy was not retried because the prior deploy attempt failed with `api-deployments-free-per-day` / `Resource is limited - try again in 24 hours`.
- Current live production remains `dpl_DHPD2Jwsk5HLbae8GyV9krgCmW6i` at `https://barmatrix-kbxxomia4-sunnylee.vercel.app`, aliased to `https://barmatrix.app` / `https://www.barmatrix.app`.
- Live `/atlas` fetch returned HTTP 200.

# Atlas V2 Subtopic Studied Progress - 2026-06-20

## Plan

- [x] Add focused regression coverage for subtopic studied progress.
- [x] Show studied progress in each grouped subtopic header.
- [x] Run focused Atlas/dashboard test, lint, build, and whitespace checks.
- [x] Commit and push to the private app repo.
- [ ] Retry production deploy when Vercel daily deploy limit resets.
...
- App commit: `3cf5861 Show Atlas subtopic studied progress`, pushed to private `auronpep/barmatrix-app` `main`.
- UI change: each grouped Atlas subtopic header now shows studied count beside total codes, ready codes, and question count.
- Proof passed: `node --test tests\ambassador-dashboard-entry.test.ts`, `git diff --check` (CRLF warnings only), `npm run lint`, and `npm run build`.
- Production deploy was not retried because the prior deploy attempt failed with `api-deployments-free-per-day` / `Resource is limited - try again in 24 hours`.
- Current live production remains `dpl_DHPD2Jwsk5HLbae8GyV9krgCmW6i` at `https://barmatrix-kbxxomia4-sunnylee.vercel.app`, aliased to `https://barmatrix.app` / `https://www.barmatrix.app`.
- Live `/atlas` fetch returned HTTP 200.

# Atlas V2 Code List Studied Badge - 2026-06-20

## Plan

- [x] Add focused regression coverage for studied state in the main Atlas code list.
- [x] Show `Studied` / `To study` directly on each main outline-code row.
- [x] Run focused Atlas/dashboard test, lint, build, and whitespace checks.
- [x] Commit and push to the private app repo.
- [ ] Retry production deploy when Vercel daily deploy limit resets.
...
- App commit: `93bcd04 Show Atlas studied state in code list`, pushed to private `auronpep/barmatrix-app` `main`.
- UI change: the main Atlas outline-code rows now show whether each code has been studied on this device, so the long outline walk is visible in the index itself.
- Proof passed: `node --test tests\ambassador-dashboard-entry.test.ts`, `git diff --check` (CRLF warnings only), `npm run lint`, and `npm run build`.
- Production deploy was not retried because the prior deploy attempt failed with `api-deployments-free-per-day` / `Resource is limited - try again in 24 hours`.
- Current live production remains `dpl_DHPD2Jwsk5HLbae8GyV9krgCmW6i` at `https://barmatrix-kbxxomia4-sunnylee.vercel.app`, aliased to `https://barmatrix.app` / `https://www.barmatrix.app`.
- Live `/atlas` fetch returned HTTP 200.
...
- [x] Add focused regression coverage for the next question-gap cue.
- [x] Show a direct jump to the first outline code in the current scope with zero approved questions.
- [x] Run focused Atlas/dashboard test, lint, build, and whitespace checks.
- [x] Commit and push to the private app repo.
- [ ] Retry production deploy when Vercel daily deploy limit resets.
...
- App commit: `1467fbd Show Atlas next question gap`, pushed to private `auronpep/barmatrix-app` `main`.
- UI change: the Weak-section drilldown card now shows `Next question gap` when the selected scope contains any outline code without an approved question.
- Proof passed: `node --test tests\ambassador-dashboard-entry.test.ts`, `git diff --check` (CRLF warnings only), `npm run lint`, and `npm run build`.
...
- [x] Add focused regression coverage for dashboard Atlas navigation copy.
- [x] Make the dashboard Atlas links describe the interactive outline-learning map.
- [x] Run focused dashboard/Atlas test, lint, build, and whitespace checks.
- [x] Commit/push/deploy if clean, then record proof here.

## Review

- App commit: `bbdc61b Clarify Atlas dashboard navigation`, pushed to private `auronpep/barmatrix-app` `main`.
- UI change: dashboard Atlas navigation now describes the route as the code-by-code lesson/question/trap/drill map, and the compact dashboard program link has a clarifying title.
- Proof passed: `node --test tests\ambassador-dashboard-entry.test.ts`, `git diff --check` (CRLF warnings only), `npm run lint`, and `npm run build`.
- Production deploy blocker: `vercel deploy --prod -y --scope sunnylee` failed with `api-deployments-free-per-day` / `Resource is limited - try again in 24 hours`.
...
- Live `/atlas` fetch returned HTTP 200.
- Pending pushed app commits not yet live: `2f25e2b Show Atlas lesson walk target`, `50709ba Show Atlas question call cue`, `820bf73 Show Atlas next gap cue`, and `bbdc61b Clarify Atlas dashboard navigation`.
...
- [x] Add a focused regression for LeadMe start feedback showing queued item count.
- [x] Include the approved item count in the successful Atlas LeadMe queue message.
- [x] Run focused Atlas/dashboard test, lint, build, and whitespace checks.
- [x] Commit/push/deploy if clean, then record proof here.

## Review

- App commit: `0063040 Show Atlas LeadMe queued item count`, pushed to private `auronpep/barmatrix-app` `main`.
- UI change: successful Atlas LeadMe start feedback now includes how many approved items were queued.
- Proof passed: `node --test tests\ambassador-dashboard-entry.test.ts`, `git diff --check` (CRLF warnings only), `npm run lint`, and `npm run build`.
- Production deploy blocker: `vercel deploy --prod -y --scope sunnylee` failed with `api-deployments-free-per-day` / `Resource is limited - try again in 24 hours`.
- Current live production remains `dpl_EHDKgUUQiAhgWgmNfP1DrouDcT2U` at `https://barmatrix-bp7v4csbh-sunnylee.vercel.app`, aliased to `https://barmatrix.app` / `https://www.barmatrix.app`.

# Atlas V2 Answer Breadcrumb Context - 2026-06-20

## Plan

- [x] Add a focused regression for the answer-debrief Atlas breadcrumb preserving the current outline code.
- [x] Route the loaded answer shell breadcrumb back to the selected code's question section.
- [x] Run focused Atlas/dashboard test, lint, build, and whitespace checks.
- [x] Commit/push/deploy if clean, then record proof here.

## Review

- App commit: `735cbfc Keep Atlas answer breadcrumb on code`, pushed to private `auronpep/barmatrix-app` `main`.
- UI change: the loaded Atlas answer-debrief shell breadcrumb now returns to that question's outline-code question section instead of generic `/atlas`.
- Proof passed: `node --test tests\ambassador-dashboard-entry.test.ts`, `git diff --check` (CRLF warnings only), `npm run lint`, and `npm run build`.
- Production deploy blocker: `vercel deploy --prod -y --scope sunnylee` failed with `api-deployments-free-per-day` / `Resource is limited - try again in 24 hours`.
- Current live production remains `dpl_EHDKgUUQiAhgWgmNfP1DrouDcT2U` at `https://barmatrix-bp7v4csbh-sunnylee.vercel.app`, aliased to `https://barmatrix.app` / `https://www.barmatrix.app`.
...
- [x] Add a focused regression for previous/next practice target positions.
- [x] Show each submitted-state previous/next question link's target position in the current code.
- [x] Run focused Atlas/dashboard test, lint, build, and whitespace checks.
- [x] Commit/push/deploy if clean, then record proof here.

## Review

- App commit: `9b828e6 Add Atlas practice neighbor cues`, pushed to private `auronpep/barmatrix-app` `main`.
- UI change: previous/next question links on the Atlas practice page now show the target question's position in the current outline code.
- Proof passed: `node --test tests\ambassador-dashboard-entry.test.ts`, `git diff --check` (CRLF warnings only), `npm run lint`, and `npm run build`.
- Production deploy blocker: `vercel deploy --prod -y --scope sunnylee` failed with `api-deployments-free-per-day` / `Resource is limited - try again in 24 hours`.
...
- [x] Add focused regressions for selected-code study sequence jumps into approved support and detour lanes.
- [x] Add `View support lanes` and `View detour lanes` links only when their existing approved counts are live.
- [x] Run focused Atlas/dashboard test, lint, build, and whitespace checks.
- [x] Commit/push/deploy if clean, then record proof here.

## Review

- App commit: `a223d10 Add Atlas study sequence lane jumps`, pushed to private `auronpep/barmatrix-app` `main`.
- UI change: selected Atlas codes now show `View support lanes` and `View detour lanes` inside the study sequence when those existing approved lanes are live.
- Proof passed: `node --test tests\ambassador-dashboard-entry.test.ts`, `git diff --check` (CRLF warnings only), `npm run lint`, and `npm run build`.
- Production deploy blocker: `vercel deploy --prod -y --scope sunnylee` failed with `api-deployments-free-per-day` / `Resource is limited - try again in 24 hours`.
- Current live production remains `dpl_Cf5DjFeLYddE5N1Aitn2ceGPnbZ8` at `https://barmatrix-lf36ikjjt-sunnylee.vercel.app`, aliased to `https://barmatrix.app` / `https://www.barmatrix.app`.

# Atlas V2 Study Sequence Debrief Bridge - 2026-06-20

## Plan

- [x] Add a focused regression for a selected-code study sequence answer-debrief action.
- [x] Add `Open answer debrief` next to `Do first question` when a code has an approved first question.
- [x] Run focused Atlas/dashboard test, lint, build, and whitespace checks.
- [x] Commit/push/deploy if clean, then record proof here.

## Review

- App commit: `cd45fb0 Add Atlas study sequence debrief bridge`, pushed to private `auronpep/barmatrix-app` `main`.
- UI change: selected Atlas codes now show `Open answer debrief` beside `Do first question` in the study sequence when an approved first question is loaded.
- Proof passed: `node --test tests\ambassador-dashboard-entry.test.ts`, `git diff --check` (CRLF warnings only), `npm run lint`, and `npm run build`.
- Production deploy blocker: `vercel deploy --prod -y --scope sunnylee` failed with `api-deployments-free-per-day` / `Resource is limited - try again in 24 hours`.
- Current live production remains `dpl_Cf5DjFeLYddE5N1Aitn2ceGPnbZ8` at `https://barmatrix-lf36ikjjt-sunnylee.vercel.app`, aliased to `https://barmatrix.app` / `https://www.barmatrix.app`.
...
- [x] Add a focused regression for a practice-page link back to the selected outline lesson.
- [x] Add a submitted-state `Study outline lesson` action next to the answer debrief and sibling-question navigation.
- [x] Run focused Atlas/dashboard test, lint, build, and whitespace checks.
- [x] Commit/push/deploy if clean, then record proof here.

## Review

- App commit: `ce0b065 Add Atlas practice lesson bridge`, pushed to private `auronpep/barmatrix-app` `main`.
- UI change: after submitting an Atlas practice answer, students now get a `Study outline lesson` action back to the selected outline code's lesson section.
- Proof passed: `node --test tests\ambassador-dashboard-entry.test.ts`, `git diff --check` (CRLF warnings only), `npm run lint`, and `npm run build`.
- Production deploy blocker: `vercel deploy --prod -y --scope sunnylee` failed with `api-deployments-free-per-day` / `Resource is limited - try again in 24 hours`.
...
# Atlas V2 Answer Page Bridge - 2026-06-20

## Plan

- [x] Add a focused regression for answer-page outline-code bridge links.
- [x] Add a compact bridge from each Atlas answer page back to the selected outline lesson, question list, and approved detours.
- [x] Run focused Atlas/dashboard test, lint, build, and whitespace checks.
- [x] Commit/push/deploy if clean, then record proof here.

## Review

- App commit: `cb162b1 Add Atlas answer detour jump`, pushed to private `auronpep/barmatrix-app` `main`.
- UI change: Atlas answer pages now show a `Review related detours` action when approved detours exist, jumping to the detour section with `#atlas-answer-detours`.
- Proof passed: `node --test tests\ambassador-dashboard-entry.test.ts`, `git diff --check` (CRLF warnings only), `npm run lint`, and `npm run build`.
- Production deploy blocker: `vercel deploy --prod -y --scope sunnylee` failed with `api-deployments-free-per-day` / `Resource is limited - try again in 24 hours`.
- Current live production remains `dpl_MYrpxt2r5PWBWKt9xGqiPDYRDsET` at `https://barmatrix-fxt9a18a7-sunnylee.vercel.app`, aliased to `https://barmatrix.app` / `https://www.barmatrix.app`.
...
- [x] Add a focused regression requiring studied-status badges inside selected subtopic rows.
- [x] Show `Studied` / `To study` on each selected-subtopic row using existing device-local progress.
- [x] Run focused Atlas test, lint, build, and whitespace checks.
- [x] Commit/push/deploy if clean, then record proof here.

## Review

- Updated `C:\barmatrix-app-atlas-answer-bridge\app\atlas\atlas-client.tsx` so each selected-subtopic row shows device-local status: `Studied` or `To study`.
- Added source regression coverage in `C:\barmatrix-app-atlas-answer-bridge\tests\ambassador-dashboard-entry.test.ts`.
- Verification passed in `C:\barmatrix-app-atlas-answer-bridge`: `node --test tests\ambassador-dashboard-entry.test.ts` (10/10), `git diff --check` (line-ending warnings only), `npm run lint`, and `npm run build`.
- Verified the GitHub target repo is private: `auronpep/barmatrix-app`.
- App commit `b01f9ae` (`Show Atlas row studied status`) was pushed to private `main`.
- Production deploy attempt was blocked by Vercel free-tier cap: `more than 100, code: "api-deployments-free-per-day"`.
- Current live production remains Vercel deployment `dpl_AtLbigfyKRn1wzzfEXqPWwYG9fdr`, URL `https://barmatrix-l1pjlih7q-sunnylee.vercel.app`, aliased to `https://barmatrix.app` and `https://www.barmatrix.app`.
...
- [x] Add a focused regression for selected-code subtopic lesson progress.
- [x] Show lesson-ready and unstudied-lesson counts in the selected-code subtopic panel.
- [x] Run focused Atlas test, lint, build, and whitespace checks.
- [x] Commit/push/deploy if clean, then record proof here.

## Review

- Updated `C:\barmatrix-app-atlas-answer-bridge\app\atlas\atlas-client.tsx` so the selected-code `This subtopic` card now shows `Subtopic lessons: {lesson-ready} / {left unstudied}`.
- Added source regression coverage in `C:\barmatrix-app-atlas-answer-bridge\tests\ambassador-dashboard-entry.test.ts`.
- Verification passed in `C:\barmatrix-app-atlas-answer-bridge`: `node --test tests\ambassador-dashboard-entry.test.ts` (10/10), `git diff --check` (line-ending warnings only), `npm run lint`, and `npm run build`.
- Verified the GitHub target repo is private: `auronpep/barmatrix-app`.
- App commit `a9f6711` (`Show Atlas subtopic lesson progress`) was pushed to private `main`.
- Production deploy attempt was blocked by Vercel free-tier cap: `more than 100, code: "api-deployments-free-per-day"`.
- Current live production remains Vercel deployment `dpl_AtLbigfyKRn1wzzfEXqPWwYG9fdr`, URL `https://barmatrix-l1pjlih7q-sunnylee.vercel.app`, aliased to `https://barmatrix.app` and `https://www.barmatrix.app`.
...
# Atlas V2 Lesson Scope Remaining - 2026-06-20

## Plan

- [x] Add a focused regression for remaining lesson-scope visibility.
- [x] Show lesson-ready and unstudied lesson counts for the current Atlas scope.
- [x] Keep this client-only; no API, database, route, or content change.
- [x] Verify, commit, push, deploy if available, and record status.

## Review

- App commit: `bdde9be` (`Show Atlas lesson scope remaining`) in `C:\barmatrix-app-atlas-answer-bridge`, pushed to private `auronpep/barmatrix-app` `main`.
- UI change: the weak-section drilldown now shows `Lessons left`, the count of lesson-ready outline codes in the current scope that are not marked studied on this device.
- Implementation: changed only `app/atlas/atlas-client.tsx` and the existing source regression in `tests/ambassador-dashboard-entry.test.ts`; no API, database, route, or content change.
...
# Atlas V2 Lesson Backlog Filter - 2026-06-20

## Plan

- [x] Add a focused regression for one-click lesson backlog filtering.
- [x] Add a scope control that shows unstudied lesson-ready outline codes.
- [x] Keep this client-only; no API, database, route, or content change.
- [x] Verify, commit, push, deploy if available, and record status.

## Review

- App commit: `ace5b64` (`Add Atlas lesson backlog filter`) in `C:\barmatrix-app-atlas-answer-bridge`, pushed to private `auronpep/barmatrix-app` `main`.
- UI change: the weak-section drilldown now has `Show lesson backlog`, which filters to lesson-ready codes and unstudied codes in one click, then selects the next lesson target.
- Implementation: changed only `app/atlas/atlas-client.tsx` and the existing source regression in `tests/ambassador-dashboard-entry.test.ts`; no API, database, route, or content change.
- Verification: the focused source regression first failed against missing `showScopedLessonBacklog`; after the patch, `node --test tests\ambassador-dashboard-entry.test.ts` passed 10/10, `git diff --check` passed with LF/CRLF warnings only, `npm run lint` passed, and `npm run build` passed.
- Production deploy blocker: `vercel deploy --prod -y --scope sunnylee` failed with `api-deployments-free-per-day` / `Resource is limited - try again in 24 hours`. Production remains `https://barmatrix-iho54q9cg-sunnylee.vercel.app` (`dpl_4yTT1J86kPGAmMp6jP1n7LgdCAT1`), so `ace5b64` is pushed but not live yet.

# Atlas V2 Scope Lesson Summary - 2026-06-20

## Plan

- [x] Add a focused regression for lesson-ready count in the scope summary.
- [x] Show lesson-ready count beside practice-ready count and question count.
- [x] Keep this client-only; no API, database, route, or content change.
- [x] Verify, commit, push, deploy if available, and record status.

## Review

- App commit: `17dd3bd` (`Show Atlas scope lesson summary`) in `C:\barmatrix-app-atlas-answer-bridge`, pushed to private `auronpep/barmatrix-app` `main`.
- UI change: the weak-section drilldown summary now shows practice-ready codes, lesson-ready codes, and approved question count together.
- Implementation: changed only `app/atlas/atlas-client.tsx` and the existing source regression in `tests/ambassador-dashboard-entry.test.ts`; no API, database, route, or content change.
