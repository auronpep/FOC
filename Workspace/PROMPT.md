...
- **component_routing.destination_key** — a controlled app destination key from the taxonomy.
- **difficulty** — carry the source row's value verbatim (e.g. "Core") when supplied; never
  substitute an invented scale.    

Free-text tags are allowed and useful in descriptive fields such as `trap_tags`,
`red_zone_dimensions`, `component_tags`, `crossovers`, drill text, and outline-mastery
fill text. Do not use free text for `component_routing.destination_key`; that field is
the stable bridge to app navigation and analytics.    
...
5. ```json``` `program_intelligence`: { question_id, subject, outline_code,    
   ...
   prompt, answer), trap_tags{forensic_tags,misconception_tags}, component_routing[] objects
   (destination_key from taxonomy, route optional, component_tags[] free text), crossovers[],    
...
JSON blocks parse** · `program_elements` uses `red_zone_dimensions` exactly, not `red_zones` ·
`program_intelligence.component_routing[]` uses controlled `destination_key` values with optional
free-text `component_tags` · cited URLs stripped of tracking parameters · Review Truth is a pure rule
