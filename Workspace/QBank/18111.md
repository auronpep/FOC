...
L2: Jules finds Taylor's blank checkbook, writes a check payable to "cash" for $800, signs Taylor's name without authority, and plans to cash it at a bank.
Stem / Fact Pattern: Jules finds Taylor's blank checkbook, writes a check payable to "cash" for $800, signs Taylor's name without authority, and plans to cash it at a bank.
Call of Question: Has Jules committed forgery?
Answer A: No, because Taylor did not actually lose money yet
Answer B: No, because checks are not writings with legal significance
Answer C: Yes, because Jules falsely made a check with apparent legal efficacy intending to defraud
Answer D: Yes, but only if Jules successfully cashes the check
Correct Answer: C
Correct Answer Explanation: Jules created a false check by signing Taylor's name without authority and intended to use it to obtain money.
Wrong Answer Explanations: A: Forgery does not require completed financial loss. The false making of a legally effective writing with intent to defraud is enough. B: A completed check purporting to draw funds has apparent legal efficacy. D: Successful uttering or theft is not required for forgery itself.
lb_rule_reference: Signing another person's name to a check without authority can be forgery because the check has apparent legal efficacy.
