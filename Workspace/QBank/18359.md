...
BARMATRIX Q#: 18359
internal_id: contracts_content_meaning_questions_batch_007_CM_C_PER_exceptions_purposes::r8::Q07
subject_display: Contracts
topic: PER exceptions and purposes
subtopic: CM-C
difficulty: Discriminator
L2: Joseph sold a set of woodworking tools to Caleb under a signed writing that listed the tools, price, and delivery date. At the same time, the parties made
Stem / Fact Pattern: Joseph sold a set of woodworking tools to Caleb under a signed writing that listed the tools, price, and delivery date. At the same time, the parties made a separate oral agreement that Caleb would rent Joseph’s empty storage shed for one month after delivery so Caleb could sort the tools. The writing did not mention the shed, and the shed rental does not contradict any written term. Joseph sues for the shed rent, and Caleb argues that the parol evidence rule bars the oral shed agreement.
Answer A: Yes, because it was made at the same time as the written tool sale.
Answer B: Yes, because any oral term connected to a signed writing is barred.
Answer C: No, if the court finds the writing was not intended as a complete and exclusive statement of the whole transaction and the shed rental was a collateral, consistent agreement not naturally expected in the tool-sale writing.
Answer D: No, but only if the tools were defective.
Correct Answer: C
Correct Answer Explanation: A consistent collateral agreement may be admissible if it does not contradict the writing and is of a kind the parties would not naturally be expected to include in that writing. The shed rental is separate from the tool sale’s core terms.
Wrong Answer Explanations: A: A is wrong because contemporaneous timing alone does not bar every collateral agreement. B: B is wrong because the parol evidence rule is not an all-purpose exclusion of oral terms. D: D is wrong because tool defects are not the basis for the collateral-agreement route.
lb_rule_reference: Common-law collateral agreement exception
